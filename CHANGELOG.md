## 1.36
- Fix: Επίλυση σφάλματος `UnboundLocalError: autoclass` που προκάλεσε κρασάρισμα στην εκκίνηση της αναγνώρισης στην 1.35.
- Update: Αναβάθμιση έκδοσης σε 1.36 στα pyproject.toml και build_apk.sh.

## 1.35
- Final Timeout Fix: Εισαγωγή της εντολής `stopListening()` αν η αναγνώριση καθυστερήσει πάνω από 8 δευτερόλεπτα. Αυτό "αναγκάζει" το Android να μας δώσει το αποτέλεσμα άμεσα.
- Stabilization: Σωστή ρύθμιση των παραμέτρων σιωπής με χρήση Java Long για ταχύτερη απόκριση.

## 1.34
- Emergency Recovery: Αφαίρεση των πειραματικών ρυθμίσεων σιωπής που προκάλεσαν το "πάγωμα" της εφαρμογής στην 1.33.
- Optimization: Αύξηση του Timeout στα 20 δευτερόλεπτα για πιο σταθερή αρχικοποίηση της αναγνώρισης.

## 1.33
- Fix News: Προσθήκη fallback parser (html.parser) αν αποτύχει ο XML, ώστε να διασφαλίζεται η ανάγνωση των ειδήσεων.
- Optimization: Μείωση χρόνου αναμονής (Timeout) στα 10 δευτερόλεπτα και ταχύτερος τερματισμός ακρόασης μόλις σταματήσετε να μιλάτε.
- UI Cleanup: Αυτόματος καθαρισμός του log σε κάθε νέα ακρόαση και αφαίρεση περιττών debug πληροφοριών περιβάλλοντος.

## 1.32
- Rename: Μετονομασία έκδοσης σε 1.32 (αλλαγή αρίθμησης).
- Robustness: Βελτίωση της αναγνώρισης ομιλίας. Πλέον, αν η αναγνώριση καθυστερήσει (Timeout), η εφαρμογή χρησιμοποιεί αυτόματα το κείμενο που "άκουσε" στα Partial Results αντί να βγάλει σφάλμα.
...
## 1.0.29
- Feature: Προσθήκη Fallback μηχανισμού για την εκτέλεση εντολών. Αν η τελική αναγνώριση (Results) αποτύχει, η εφαρμογή χρησιμοποιεί το τελευταίο κείμενο από τα Partial Results.
- Feature: Νέες εντολές για Ώρα ("τι ώρα είναι") και Ημερομηνία ("τι μέρα έχουμε").
- Fix: Προσθήκη ρητής καταγραφής (Executing: ...) για επιβεβαίωση της εκτέλεσης εντολών.

## 1.0.28
- Feature: Βελτιστοποίηση της αναγνώρισης ομιλίας (STT) με προσθήκη Partial Results (εμφάνιση κειμένου ενώ μιλάτε) και βελτιωμένη διαχείριση σφαλμάτων (φιλικότερα μηνύματα για No Match/Error 7).
- Fix: Αύξηση του timeout αναμονής στα 30 δευτερόλεπτα για καλύτερη εμπειρία χρήστη.
